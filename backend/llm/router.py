...
Adapted from the screencapture project's synchronous router to async for FastAPI.
"""
...
from llm.models import MODELS, ModelInfo
from llm.providers import BASE_URLS, KEY_ENV_VARS, NO_PENALTY_PROVIDERS, all_keys
...
class RoutedLLMBackend:
    ...
    async def route(
        ...
        """Attempt models in order. Raises RuntimeError if every attempt failed."""
        order = self.chain()
        ...
        for candidate in order:
            ...
            for key_pass in range(key_count):
                ...
                try:
                    text = await self._call(candidate, messages, temperature, max_tokens)
    ...
    async def _call(
        ...
        # Strip reasoning model <think> blocks before checking emptiness
